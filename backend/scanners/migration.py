...
class MigrationScanner:
    ...
    def checks(self) -> list[ScanCheck]:
        """Return the full catalogue of migration readiness checks."""
    ...
    def evaluate(self, data: RepoAssessmentData) -> list[CheckResult]:
        ...
        check = check_map["MIG-001"]
        if data.has_migration_guide:
            results.append(
                CheckResult(
                    check=check,
                    status=CheckStatus.passed,
                    detail="A migration guide is present in the repository.",
                )
            )
        else:
            results.append(
                CheckResult(
                    ...
                    detail="No migration guide was found.",
                )
            )

        # MIG-002 — API versioning implemented (proxied via API docs presence)
        check = check_map["MIG-002"]
        if data.has_api_docs:
            results.append(
                CheckResult(
                    check=check,
                    status=CheckStatus.passed,
                    detail="API documentation is present, indicating API versioning is likely implemented.",
                )
            )
        else:
            results.append(
                CheckResult(
                    check=check,
                    status=CheckStatus.failed,
                    detail="No API documentation found; API versioning may not be implemented.",
                )
            )

        # MIG-003 — Deprecation policy documented
        check = check_map["MIG-003"]
        if data.has_deprecation_policy:
            results.append(
                CheckResult(
                    check=check,
                    status=CheckStatus.passed,
                    detail="A deprecation policy document is present in the repository.",
                )
            )
        else:
            results.append(
                CheckResult(
                    check=check,
                    status=CheckStatus.failed,
                    detail="No deprecation policy document was found.",
                )
            )

        # MIG-004 — Database migration tool configured (not verifiable via standard API)
        check = check_map["MIG-004"]
        results.append(
            CheckResult(
                check=check,
                status=CheckStatus.warning,
                detail=(
                    "Database migration tool configuration cannot be verified automatically via the "
                    "repository API. Manual review of project dependencies and configuration is recommended."
                ),
            )
        )

        # MIG-005 — Feature parity documentation (not verifiable via standard API)
        check = check_map["MIG-005"]
        results.append(
            CheckResult(
                check=check,
                status=CheckStatus.warning,
                detail=(
                    "Feature parity documentation cannot be detected automatically. "
                    "Manual review recommended to confirm equivalent capabilities are documented."
                ),
            )
        )

        # MIG-006 — Data export capability (not verifiable via standard API)
        check = check_map["MIG-006"]
        results.append(
            CheckResult(
                check=check,
                status=CheckStatus.warning,
                detail=(
                    "Data export capability cannot be verified automatically from repository metadata. "
                    "Manual review of application features and documentation is recommended."
                ),
            )
        )

        # MIG-007 — Backwards compatibility testing (not verifiable via standard API)
        check = check_map["MIG-007"]
        results.append(
            CheckResult(
                check=check,
                status=CheckStatus.warning,
                detail=(
                    "Backwards compatibility test coverage cannot be determined automatically. "
                    "Manual review of the test suite is recommended to confirm compatibility tests exist."
                ),
            )
        )

        # MIG-008 — Environment parity (not verifiable via standard API)
        check = check_map["MIG-008"]
        results.append(
            CheckResult(
                check=check,
                status=CheckStatus.warning,
                detail=(
                    "Environment parity between development, staging, and production cannot be verified "
                    "via the repository API. Manual review of deployment configuration is recommended."
                ),
            )
        )

        # MIG-009 — Platform abstraction layer present (not verifiable via standard API)
        check = check_map["MIG-009"]
        results.append(
            CheckResult(
                check=check,
                status=CheckStatus.warning,
                detail=(
                    "The presence of a platform abstraction layer cannot be determined automatically "
                    "from repository metadata. Manual review of the codebase architecture is recommended."
                ),
            )
        )
